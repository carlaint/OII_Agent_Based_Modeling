...
to generate-clusters
  let cluster-std-dev 20 - 2
  let cluster-size num-data-points / 2
  repeat 2 [
...
;;----------------
;; 2. make dense network for each blob
;;----------------

to create-random
setup
  let current-degree 0
    while [current-degree < AvDegree] [
      add-edge
      set current-degree (2 * count links / count turtles)
    ]
end
@#$#@#$#@
GRAPHICS-WINDOW
